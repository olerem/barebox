...
static int NandWaitForControllerReady(void __iomem *iobase)
{
	...
	while (ioread32(iobase + HW_STATUS) & (1UL << 8))
	{
		waittime++;
...
static int NandWaitForDeviceReady(void __iomem *iobase, u8 nChip)
{
	...
	while (!(ioread32(iobase + HW_STATUS) & (1UL << nChip)))
	{
		waittime++;
		if (waittime > 0x1000000)
		{
...
static void NandReadBuffer(void __iomem *iobase, unsigned char *pBuffer, u32 nLen)
{
	...
	for (i = 0; i < (nLen>>2); i++)
	{
		tmpbuf[i] = ioread32(iobase + HW_FIFO_DATA);
	}
...
{
    iowrite32((0xff00 | nChip), iobase + HW_MEM_CTRL);
    iowrite32( (RESET << 8)
        | (ADDR_SEL_0 << 7)
        | (INPUT_SEL_BIU << 6)
        | (SEQ0), iobase + HW_CMD);
...
static int NandTimingConf(void __iomem *iobase)
{
	...
	iowrite32((ADDR_CYCLE_1 << 18) 		| (ADDR1_AUTO_INCR_DIS << 17)
	   ...
	   | (ECC_DIS << 5) 				| (INT_DIS << 4)
	   | (SPARE_DIS << 3) 			| (ADDR_CYCLE_1), iobase + HW_CTRL); 
#if 0
...
{
    iowrite32((0xff00 |  nChip), iobase + HW_MEM_CTRL);
    iowrite32((1UL << (nChip+8)) ^ ioread32(iobase + HW_MEM_CTRL), iobase + HW_MEM_CTRL);// clear WP reg
}
...
int NandReadIDPio(void __iomem *iobase, unsigned char nChip, unsigned int *pDevId, unsigned int nByte)
{
	...
	iowrite32(1, iobase + HW_FIFO_INIT);		/*复位FIFO*/
	iowrite32(nByte, iobase + HW_DATA_SIZE);
	iowrite32((0xff00 | nChip), iobase + HW_MEM_CTRL);
	iowrite32(0x00000000, iobase + HW_ADDR0_0);
	iowrite32((READ_ID << 8)
	   | (ADDR_SEL_0 << 7)
	   | (INPUT_SEL_BIU << 6)
	   | (SEQ1), iobase + HW_CMD);
	...
	pDevId[0] = ioread32(iobase + HW_FIFO_DATA);
...
static void NandControllerConfig (void __iomem *iobase)
{
	...
	iowrite32((EN_STATUS << 23)				| (NO_RNB_SEL << 22)
	   ...
	   | (pNandInfo->addr_cycles), iobase + HW_CTRL);
...
static int NandReadLargePagePio(void __iomem *iobase, u32 nPage, u32 nColumn, unsigned char *pBuffer, u32 nLen)
{
	...
	iowrite32(0x1, iobase + HW_FIFO_INIT);
	...
	iowrite32((0xff00 | pNandInfo->chip_select), iobase + HW_MEM_CTRL);
	NandLargePageMakeAddr(nPage, nColumn, NandAddr);
	iowrite32(addr[0], iobase + HW_ADDR0_0);
	iowrite32(addr[1], iobase + HW_ADDR0_1);
	...
	if ((ioread32(iobase + HW_CTRL) & (ECC_EN<<5)) != 0)
	{
		ret = NandEccCheck( );
	}
	return ret;
